...
CREATE OR REPLACE FUNCTION tm_cz.I2B2_CREATE_FULL_TREE(path character varying, currentjobid numeric DEFAULT (-1))
RETURNS numeric AS
$BODY$
DECLARE
	parent_c CURSOR FOR
	SELECT p.PATH, p.record_id, p.PATH_LEN FROM TM_WZ.I2B2_LOAD_PATH p;
	rowCt			numeric(18,0);
...
BEGIN
	...
	DELETE FROM TM_WZ.I2B2_LOAD_PATH;

	--Remove duplicates
	INSERT INTO TM_WZ.I2B2_LOAD_PATH(PATH, RECORD_ID, PATH_LEN)
	...
	UPDATE TM_WZ.I2B2_LOAD_PATH p SET PATH200 = SUBSTR(p.PATH, 1, 200), PATH150 = SUBSTR(p.PATH, 1, 150), PATH100 = SUBSTR(p.PATH, 1, 100), PATH50 = SUBSTR(p.PATH, 1, 50);

	ANALYZE TM_WZ.I2B2_LOAD_PATH;

	get diagnostics rowCt := ROW_COUNT;
	select tm_cz.cz_write_audit(currentjobid, databaseName, procedureName,'Inserted into I2B2_LOAD_PATH',rowCt,0,'Done') into rtnCd;
	...
	DROP INDEX IF EXISTS TM_WZ.TM_WZ_IDX_ROOT;
	DROP INDEX IF EXISTS TM_WZ.TM_WZ_IDX_CHILD;
	DELETE FROM TM_WZ.I2B2_LOAD_TREE_FULL;

	/*FOR p IN parent_c LOOP
		BEGIN
			INSERT INTO TM_WZ.I2B2_LOAD_TREE_FULL(IDROOT, IDCHILD)
			SELECT p.RECORD_ID, c.RECORD_ID
			from TM_WZ.I2B2_LOAD_PATH c
	...
	INSERT INTO TM_WZ.I2B2_LOAD_TREE_FULL(IDROOT, IDCHILD)
	SELECT p.RECORD_ID, c.RECORD_ID
	from TM_WZ.I2B2_LOAD_PATH c, TM_WZ.I2B2_LOAD_PATH p
	...
	SELECT COUNT(*) FROM TM_WZ.I2B2_LOAD_TREE_FULL INTO rowCt;
	--get diagnostics rowCt := ROW_COUNT;
	select tm_cz.cz_write_audit(currentjobid, databaseName, procedureName,'Inserted into I2B2_LOAD_TREE_FULL',rowCt,0,'Done') into rtnCd;
