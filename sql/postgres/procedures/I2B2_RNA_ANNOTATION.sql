CREATE OR REPLACE FUNCTION i2b2_rna_annotation(gpl_id character varying, currentjobid int DEFAULT NULL::int)
  RETURNS numeric AS
  $BODY$
DECLARE
...
BEGIN
	...
	stepCt := stepCt + 1;
	select cz_write_audit(jobId,databaseName,procedureName,'Starting i2b2_rna_annotation',0,stepCt,'Done') into rtnCd;
	...
	begin
		...
		(
			...
			SELECT DISTINCT
				...
				NULL,
				a.organism
			FROM LT_RNA_ANNOTATION a
			WHERE
				a.transcript_id NOT IN (SELECT DISTINCT transcript_id
																FROM deapp.DE_RNA_ANNOTATION);
  ...
  begin
    ...
    update deapp.DE_RNA_ANNOTATION a set GENE_ID=(select primary_external_id from biomart.bio_marker b where
	...
	stepCt := stepCt + 1;
	select cz_write_audit(jobId,databaseName,procedureName,'End i2b2_rna_annotation',0,stepCt,'Done') into rtnCd;
