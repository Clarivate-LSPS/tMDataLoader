--
-- Name: i2b2_proteomics_zscore_calc(character varying, character varying, numeric, character varying, numeric, character varying); Type: FUNCTION; Schema: tm_dataloader; Owner: -
--
CREATE OR REPLACE FUNCTION i2b2_proteomics_zscore_calc(trial_id character varying, run_type character varying DEFAULT 'L'::character varying, currentjobid numeric DEFAULT NULL::numeric, data_type character varying DEFAULT 'R'::character varying, log_base numeric DEFAULT 2, source_cd character varying DEFAULT NULL::character varying) RETURNS numeric
...
--	truncate tmp tables
	begin
		execute ('truncate table WT_SUBJECT_PROTEOMICS_LOGS');
		execute ('truncate table WT_SUBJECT_PROTEOMICS_CALCS');
		execute ('truncate table WT_SUBJECT_PROTEOMICS_MED');
	exception
	when others then
	...
	--drop index if exists WT_SUBJECT_PROTEOMICS_LOGS_I1;		
	--drop index if exists WT_SUBJECT_PROTEOMICS_CALCS_I1;
	
	stepCt := stepCt + 1;
	select cz_write_audit(jobId,databaseName,procedureName,'Truncate work tables',0,stepCt,'Done') into rtnCd;
	...
	stepCt := stepCt + 1;
	get diagnostics rowCt := ROW_COUNT;
	select cz_write_audit(jobId,databaseName,procedureName,'Loaded data for trial in wt_subject_mirna_logs',rowCt,stepCt,'Done') into rtnCd;

	
    
	--execute ('create index WT_SUBJECT_PROTEOMICS_LOGS_I1 on WT_SUBJECT_PROTEOMICS_LOGS (trial_name, probeset_id)');
	stepCt := stepCt + 1;
	--select cz_write_audit(jobId,databaseName,procedureName,'Create index WT_SUBJECT_PROTEOMICS_LOGS_I1',0,stepCt,'Done') into rtnCd;
...
--	calculate mean_intensity, median_intensity, and stddev_intensity per experiment, probe
	...
	stepCt := stepCt + 1;
	get diagnostics rowCt := ROW_COUNT;
	select cz_write_audit(jobId,databaseName,procedureName,'Calculate intensities for trial in WT_SUBJECT_PROTEOMICS_CALCS',rowCt,stepCt,'Done') into rtnCd;

	

	--execute ('create index wt_subject_proteomics_calcs_i1 on WT_SUBJECT_PROTEOMICS_CALCS (trial_name, probeset_id) nologging tablespace "INDX"');
	--stepCt := stepCt + 1;
	--cz_write_audit(jobId,databaseName,procedureName,'Create index on WT_SUBJECT_PROTEOMICS_CALCS',0,stepCt,'Done');
...
--	,sample_cd
	...
	stepCt := stepCt + 1;
	get diagnostics rowCt := ROW_COUNT;
	select cz_write_audit(jobId,databaseName,procedureName,'Calculate Z-Score for trial in WT_SUBJECT_PROTEOMICS_MED',rowCt,stepCt,'Done') into rtnCd;
