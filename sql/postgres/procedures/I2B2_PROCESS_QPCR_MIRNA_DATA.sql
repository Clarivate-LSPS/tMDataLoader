CREATE OR REPLACE FUNCTION i2b2_process_qpcr_mirna_data(
	...
	source_cd            CHARACTER VARYING DEFAULT 'STD' :: CHARACTER VARYING, log_base NUMERIC DEFAULT 2,
	secure_study         CHARACTER VARYING DEFAULT NULL :: CHARACTER VARYING,
	currentjobid         NUMERIC DEFAULT NULL :: NUMERIC, mirna_type CHARACTER VARYING DEFAULT NULL :: CHARACTER VARYING,
		...
		if dataType = 'R' or dataType = 'L' then
			begin
				if mirna_type='MIRNA_QPCR' then
					perform i2b2_mirna_zscore_calc(TrialID,'L',jobId,'C',logBase,sourceCD);----donot do log transform
				else
					perform i2b2_mirna_zscore_calc(TrialID,'L',jobId,dataType,logBase,sourceCD);----do log transform
				end if;
				stepCt := stepCt + 1; get diagnostics rowCt := ROW_COUNT;
