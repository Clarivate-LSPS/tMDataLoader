-- DROP FUNCTION i2b2_process_acgh_data(character varying, character varying, character varying, character varying, numeric);

CREATE OR REPLACE FUNCTION i2b2_process_acgh_data(trial_id character varying, top_node character varying, source_cd character varying DEFAULT 'STD'::character varying, secure_study character varying DEFAULT 'N'::character varying, currentjobid numeric DEFAULT (-1))
  RETURNS numeric AS
$BODY$
/*************************************************************************
...
BEGIN
	...
	stepCt := 0;
	stepCt := stepCt + 1;
	select cz_write_audit(jobId,databaseName,procedureName,'Starting i2b2_process_acgh_data',0,stepCt,'Done') into rtnCd;
	...
	if pCount = 0 then
		select cz_write_audit(jobId,databaseName,procedureName,'No Chromosomal platforms in deapp.de_chromosomal_region',0,pCount,'Done') into rtnCd;
		select cz_error_handler (jobID, procedureName, '-1', 'Application raised error') into rtnCd;
		select cz_end_audit (jobID, 'FAIL') into rtnCd;
		return -16;
	end if;
	...
	where sm.trial_name = trial_id
	  ...
	  and sm.platform = 'ACGH'
	 and not exists
		 (select 1 from lt_src_mrna_subj_samp_map x
	...
	stepCt := stepCt + 1;
	select cz_write_audit(jobId,databaseName,procedureName,'Delete dropped site/subject/sample from de_subject_sample_mapping',rowCt,stepCt,'Done') into rtnCd;
	...
	stepCt := stepCt + 1;
	select cz_write_audit(jobId,databaseName,procedureName,'Initialize visualattributes and xml in i2b2',rowCt,stepCt,'Done') into rtnCd;
  ...
  -- a unique top-level node for mRNA data
	...
	execute ('truncate table wt_subject_acgh_region');

	--	note: assay_id represents a unique subject/site/sample

	begin
	insert into wt_subject_acgh_region
	(region_id
	,chip
	,segmented
	,flag
	...
	stepCt := stepCt + 1;
	select cz_write_audit(jobId,databaseName,procedureName,'Insert into DEAPP wt_subject_acgh_region',rowCt,stepCt,'Done') into rtnCd;
	...
	stepCt := stepCt + 1;
	select cz_write_audit(jobId,databaseName,procedureName,'End i2b2_process_acgh_data',0,stepCt,'Done') into rtnCd;
