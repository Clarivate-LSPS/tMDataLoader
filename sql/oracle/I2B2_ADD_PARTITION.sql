...
-- True
		...
		THEN
			...
			cz_write_audit(jobId, databaseName, procedureName,
										 'Partitioning is not available for ' || qualifiedName || '. Skipping...', 0,
										 stepCt, 'Done');
			COMMIT;
		...
		THEN
			stepCt := stepCt + 1;
			cz_write_audit(jobId, databaseName, procedureName,
										 'Partition by ' || partitionName || ' already exists for ' || qualifiedName || '. Skipping...', 0,
										 stepCt, 'Done');
			COMMIT;
		...
		THEN
			...
			stepCt := stepCt + 1;
			cz_write_audit(jobId, databaseName, procedureName, 'Added partition to ' || qualifiedName, 0, stepCt, 'Done');

			n := 0;
			SELECT count(*)
			INTO cnt
			FROM all_indexes
			WHERE table_name = tbl_name AND table_owner = tbl_owner AND partitioned = 'YES';
			...
			THEN
				...
				LOOP
					...
					stepCt := stepCt + 1;
					cz_write_audit(jobId, databaseName, procedureName,
												 'Rebuilt index ' || n || ' of ' || cnt || ' ' || qualifiedName || ' partition indexes', 0,
												 stepCt, 'Done');
					COMMIT;
				END LOOP;
			END IF;
