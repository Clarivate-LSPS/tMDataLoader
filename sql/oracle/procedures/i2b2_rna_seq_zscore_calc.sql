...
--		end if;
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Loaded data for trial in TM_DATALOADER wt_subject_rna_logs',SQL%ROWCOUNT,stepCt,'Done');
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Create index on TM_DATALOADER WT_SUBJECT_RNA_LOGS_I1',0,stepCt,'Done');
...
--	calculate mean_intensity, median_intensity, and stddev_intensity per experiment, probe
	...
	group by d.trial_name 
			,d.probeset_id;
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Calculate intensities for trial in TM_DATALOADER wt_subject_rna_calcs',SQL%ROWCOUNT,stepCt,'Done');
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Create index on TM_DATALOADER WT_SUBJECT_RNA_CALCS_I1',0,stepCt,'Done');
...
--	,subject_id
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Calculate Z-Score for trial in TM_DATALOADER wt_subject_rna_med',SQL%ROWCOUNT,stepCt,'Done');
