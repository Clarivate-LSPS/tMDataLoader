...
as
  ...
  BEGIN
    ...
    stepCt := 0;
    stepCt := stepCt + 1;
    cz_write_audit(jobId,databaseName,procedureName,'Starting ' || procedureName || ' '||trial_id|| ' '||topNode||' '||sourceCd||' '||secure_study,0,stepCt,'Done');
    ...
    delete from de_subject_sample_mapping where
      assay_id in (
        ...
        where
          ...
          and dssm.platform = platform_type
      );
    stepCt := stepCt + 1;
      ...
      from (select distinct b.patient_num as patient_id
            ...
            from lt_src_mrna_subj_samp_map a
              ...
              left join wt_mrna_nodes pn
                on nvl(a.platform, '@') = nvl(pn.platform, '@')
    ...
    where a.c_basecode in (
      select distinct x.concept_code from de_subject_sample_mapping x
      where x.trial_name = TrialId
          and x.platform = platform_type
