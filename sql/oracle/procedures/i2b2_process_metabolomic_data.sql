--
-- Type: PROCEDURE; Owner: TM_DATALOADER; Name: I2B2_PROCESS_METABOLOMIC_DATA
--
  CREATE OR REPLACE PROCEDURE "TM_DATALOADER"."I2B2_PROCESS_METABOLOMIC_DATA" 
(
		trial_id                 VARCHAR2
	, top_node                 VARCHAR2
...
--	cursor to define the path for delete_one_node  this will delete any nodes that are hidden after i2b2_create_concept_counts
  ...
  where c_fullname like topNode || '%'
    ...
    tm_dataloader.LT_METABOLOMIC_DISPLAY_MAPPING;
...
--	truncate tmp node table
	...
	execute immediate('truncate table tm_dataloader.WT_METABOLOMIC_NODES');
...
--	from wt_subject_metabolomics_data
	...
	execute immediate('truncate table tm_dataloader.WT_METABOLOMIC_NODE_VALUES');
...
--	update concept_cd for nodes, this is done to make the next insert easier
  ...
  --ASSAY_ID        = generated by trigger
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Insert trial into DEAPP de_subject_sample_mapping s= ' || sourceCd || ' t= ' || TrialID,SQL%ROWCOUNT,stepCt,'Done');
...
--	Insert records for patients and samples into observation_fact
    ...
    select distinct m.sample_id
			,m.sample_id
		  ,m.concept_code
		  ,'TRANSMART:HIGHDIM:METABOLOMICS'
		  ,'T' -- Text data type
		  ,'E'  --Stands for Equals for Text Types
...
*/
 ...
 ---INSERT sample_dimension
	...
	SET n.c_columndatatype = 'T',
      ...
      from tm_dataloader.LT_METABOLOMIC_DISPLAY_MAPPING m where m.category_cd=ul.category_cd)||
...
--	tag data with probeset_id from reference.probeset_deapp
	...
	execute immediate ('truncate table tm_dataloader.WT_SUBJECT_MBOLOMICS_PROBESET');
