CREATE OR REPLACE PROCEDURE "I2B2_PROCESS_SNP_DATA"
(
  trial_id 		VARCHAR2
 ,top_node		varchar2
...
--	truncate tmp node table
	...
	execute immediate('truncate table TM_DATALOADER.wt_mrna_nodes');
...
--	from wt_subject_mrna_data
	...
	execute immediate('truncate table TM_DATALOADER.wt_mrna_node_values');
...
*/
  ...
  --Reload Security: Inserts one record for every I2B2 record into the security table
	...
	I2B2_UNUSABLE_GLOBAL_INDEXES('DEAPP', 'DE_SAMPLE_SNP_DATA', job_id=>jobID);
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Cleanup de_sample_snp_data',0,stepCt,'Done');
	COMMIT;
	...
	stepCt := stepCt + 1;
	cz_write_audit(jobId,databaseName,procedureName,'Insert into de_sample_snp_data',0,stepCt,'Done');
  COMMIT;

	I2B2_REBUILD_GLOBAL_INDEXES('DEAPP', 'DE_SAMPLE_SNP_DATA', job_id=>jobID);
  ...
  COMMIT;
	...
	if cnCount > 0 then
		execute immediate ('truncate table TM_DATALOADER.lt_src_mrna_data');
		commit;
