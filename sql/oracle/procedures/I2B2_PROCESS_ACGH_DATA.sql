...
BEGIN
  ...
  cz_write_audit(jobId,databaseName,procedureName,
								 'Tech info [shared_patients: ' || nvl(shared_patients,'NULL') || ', strong_patient_check: '|| NVL(strong_patient_check,'NULL') || ']',0,stepCt,'Done');
 ...
 -- check if platform exists in de_chromosomal_region .  If not, abort run.
  ...
  execute immediate ('truncate table TM_DATALOADER.wt_subject_acgh_region');
  ...
  cz_write_audit(jobId,databaseName,procedureName,'Insert into DEAPP wt_subject_acgh_region',pExists,stepCt,'Done');
  commit;
