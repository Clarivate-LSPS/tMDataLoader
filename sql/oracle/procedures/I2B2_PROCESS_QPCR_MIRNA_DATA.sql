...
--	cursor to define the path for delete_one_node  this will delete any nodes that are hidden after i2b2_create_concept_counts
  ...
  where c_fullname like topNode || '%'
    ...
    tm_dataloader.lt_src_mirna_display_mapping;
...
--	Insert records for patients and samples into observation_fact
   ...
   for ul in uploadI2b2
	...
	SET n.c_columndatatype = 'T',
      ...
      from tm_dataloader.lt_src_mirna_display_mapping m where m.category_cd=ul.category_cd)||
...
--	tag data with probeset_id from reference.probeset_deapp
	...
	execute immediate ('truncate table tm_dataloader.WT_SUBJECT_MIRNA_PROBESET');

        analyze_table(user, 'MIRNA_PROBESET_DEAPP', jobId);
        analyze_table('TM_DATALOADER', 'LT_SRC_QPCR_MIRNA_DATA', jobId);
        analyze_table('TM_DATALOADER', 'LT_SRC_MIRNA_SUBJ_SAMP_MAP', jobId);
