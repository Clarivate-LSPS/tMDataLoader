		...
		if (processFiles(dir, sql, studyInfo)) {
			...
			sql.call('{call tm_cz.cz_start_audit(?,?,?)}', [getProcedureName(), config.db.username, Sql.NUMERIC]) {
				...
				while (true) {
					...
					ctrlSql.eachRow("SELECT * FROM tm_cz.cz_job_audit WHERE job_id=${jobId} and seq_id>${lastSeqId} order by seq_id") {
						row ->
				...
				sql.eachRow("SELECT * FROM tm_cz.cz_job_error where job_id=${jobId} order by seq_id") {
				...
				if (res) {
					...
					sql.call("{call tm_cz.cz_end_audit(?,?)}", [jobId, 'SUCCESS'])
				}
				else {
